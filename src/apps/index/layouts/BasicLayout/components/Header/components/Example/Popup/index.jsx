...
import { appspost } from "@indexApi";
import './tionpopup.css';

const { Row, Col } = Grid;
export default class Tionpopup extends Component {
  constructor(props) {
    super(props);
    this.state = {
      open: false,
  ...
  addip() {
    const appName = this.input.value;
    appspost({
      appName,
    }).then(({ status,data })=>{
      debugger;
    ...
    return (
      <div className='ippopup'>
        <div className='ippopup-top'>
          <h2>添加应用</h2>
          <strong onClick={this.tionpopupclose.bind(this)}>×</strong>
        </div>
        ...
        <div className='ippopup-btn'>
          <button onClick={this.tionpopupclose.bind(this)}>取消</button>
